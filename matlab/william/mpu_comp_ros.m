...
% BIAS Calculation
    ...
    %PRECISO AJEITAR A PARTE DA CALIBRAÇÃO
    display('Coletando dados para a calibração');
    [accx_0,accy_0,accz_0,gyrox_0,gyroy_0,gyroz_0] = bias_calculation(ROS_MASTER_IP,TOPIC,tam,n);
...
while(1)
   ...
   %% Aquisição de dados
   accx = msg.LinearAcceleration.X + accx_0;
   accy = msg.LinearAcceleration.Y + accy_0;
   accz = msg.LinearAcceleration.Z + accz_0;
   
   gyrox(2) = msg.AngularVelocity.X + gyrox_0;
   gyroy(2) = msg.AngularVelocity.Y + gyroy_0;
   gyroz(2) = msg.AngularVelocity.Z + gyroz_0;
   
   %% Calculando ângulos
      gyro_y(2) = (gyro_y(1)+gyroy(1)*1/freq); 
      gyro_x(2) =(gyro_x(1)+gyrox(1)*1/freq);
      
    acc_p = -atan2(accx,sqrt(accz*accz+accy*accy));
    acc_r = atan2(accy,sqrt(accz*accz+accx*accx));
      ...
      cf_roll(2) = a*(cf_roll(1)+gyro_x(2)-gyro_x(1))+(1-a)*acc_r;  
        ...
        if(time>=0)
            time = 0;
            [flag] = flag_movimento(accx,accy,accz,flag);
        end
