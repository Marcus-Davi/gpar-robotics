...
var_pitch = 0.025; %depois calcular a variância de fato
var_roll = 0.025; %depois calcular a variância de fato
...
gyro_x = zeros(tam,1);
for i = 2:tam
   gyro_y(i) = gyro_y(i-1) + T*gyroy(i); %Nossa estimativa, fica no X
   gyro_x(i) = gyro_x(i-1) + T*gyrox(i);
end

[pitch roll] = linear_aceleration(pitch,roll,gyro_y,gyro_x);
